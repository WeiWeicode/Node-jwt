...
exports.signup = (req, res) => {
  ...
  // bcrypt 是一個幫助你 hash 密碼的函式庫。
    .then(user => {
      // then() 方法返回一個 Promise。它用於指定一個 Promise 成功時的回調函式。
      // 這裡的 user 是一個物件，包含了剛剛新增的使用者資料。
      if (req.body.roles) {
        ...
        Role.findAll({
          where: {
            name: {
              ...
              // [Op.or] 是 Sequelize 的運算子，用來指定 where 條件的邏輯運算子。
        ...
        // roles = table名稱
          ...
          user.setRoles(roles).then(() => {
            // setRoles 設置角色
            // user.setRoles(roles) 是 Sequelize 的方法，用來設置使用者的角色。
